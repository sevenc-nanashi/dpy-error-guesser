...
`.trim()
RESULT_PLACEHOLDER = `
discord.pyを使ったコードが動かない原因をコードから推測するWebツールです。
...
ERRORS = [
    ...
    [/commands\.Bot[\s\S]*discord\.Client|discord\.Client[\s\S]*commands\.Bot/g, "clientはbotに置き換えることができます。"],
    [/(?<=(.+)=commands\.Bot[\s\S]*)@\1.event\nasync def on_message(?![\s\S]*process_commands)/g, "$?コマンドを実行するにはprocess_commandsが必要です。"],
    ...
    get format() {
        if (this.message.startsWith("$")) {
            ...
            if (this.message.charAt(1) == "?") {
                return `（${this.message.substr(2)}）`
            } else {
                return `（${this.message.substr(1)}）`
            }
...
function detectCode() {
    content = document.getElementById("main-code-textarea").value.replaceAll(/ *([,=]) */g, "$1").replaceAll(/^ +/g, "").replaceAll(/#.*/g, "").replaceAll(/\n{2,}/g, "\n")
    if (content.length == 0) {
        document.getElementById("result-code-textarea").value = ""
        return
    }
    document.getElementById("result-code-textarea").value = "検出中です..."
    code = ""
    inQuote = null
    bracketCounter = 0
    lineCounter = 1
    ...
    results = []
    eventCounter = {}
    showVerbose = document.getElementById("result-show-verbose-input").checked
    for ([regex, message] of ERRORS) {
        for (match of code.matchAll(regex)) {
            ...
            if (!showVerbose && (message.startsWith("?") || message.charAt(1) == "?")) {
                continue
            } else {
